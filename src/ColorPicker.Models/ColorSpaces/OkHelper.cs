...
namespace ColorPicker.Models.ColorSpaces
{
	public static class OkHelper
	{
...
#if NET451
		private static double Cbrt(double value) => Math.Pow(value, 1.0 / 3.0);
#else
		private static double Cbrt(double value) => Math.Cbrt(value);
#endif
		...
		private static LC FindCusp(double a, double b)
		{
			...
			double L_cusp = Cbrt(1.0 / Math.Max(Math.Max(rgb_at_max.R, rgb_at_max.G), rgb_at_max.B));
		...
		public static Rgb OkHsvToSrgb(double h, double s, double v)
		{
			...
			double scale_L = Cbrt(1.0 / Math.Max(Math.Max(rgb_scale.R, rgb_scale.G), Math.Max(rgb_scale.B, 0.0)));
