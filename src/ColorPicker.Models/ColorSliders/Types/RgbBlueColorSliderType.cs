...
namespace ColorPicker.Models.ColorSliders.Types
{
    internal class RgbBlueColorSliderType : IColorSliderType
    {
        public List<ColorSliderGradientPoint> CalculateRgbGradient(ColorState state, bool enabled)
        {
            ...
            {
                if (enabled)
                    return new ColorSliderGradientPoint(state.RGB_R, state.RGB_G, value, value);
                else
                    return new ColorSliderGradientPoint((Colors.Rgb)ColorSpaceHelper.RgbToGrayTuple(state.RGB_R, state.RGB_G, value), value);
            }
        }
